...
public class KafkaPartitionTestUtil {
     ...
     * @param numMsgs
     */
    public static void checkResultsAgainstSkew(
            PartitionTestScenario scenario,
        ...
        for (int ptn = 0; ptn < numPtns; ptn++) {
            ...
            if (scenario == PartitionTestScenario.PARTITION_ID_HEADER_ONLY
                ...
                Assert.assertEquals(
                        "Scenario: " + scenario + " Partition " + ptn + " incorrect",
                        expectedResults.size(),
                        actualResults.size());
                // Go and check the message payload is what we wanted it to be
                for (int idx = 0; idx < expectedResults.size(); idx++) {
                    Assert.assertArrayEquals(
                            "Scenario: " + scenario + " Partition " + ptn + " event " + idx + " incorrect",
                            expectedResults.get(idx).getBody(),
                ...
                if (ptn == staticPtn) {
                    Assert.assertEquals(
                            "Scenario: " + scenario + " incorrect number of messages in partition " + ptn,
                            numMsgs,
                            actualResults.size());
                } else {
                    Assert.assertEquals(
                            "Scenario: " + scenario + " partition " + ptn + "should have no messages",
                            0,
                            actualResults.size());
                }
            }
     ...
     * @return
     */
    public static List<Event> generateSkewedMessageList(
            PartitionTestScenario scenario,
            int numMsgs,
            Map<Integer, List<Event>> partitionMap,
            int numPtns,
     ...
     */
    public static Map<Integer, List<byte[]>> retrieveRecordsFromPartitions(
            String topic, int numPtns, Properties consumerProperties) {
        ...
        for (int i = 0; i < numPtns; i++) {
            ...
            KafkaConsumer<String, byte[]> consumer = new KafkaConsumer<String, byte[]>(consumerProperties);
