...
public class TestKafkaSource {
    ...
    @SuppressWarnings("unchecked")
    @Test
    public void testProcessItNotEmpty()
            throws EventDeliveryException, SecurityException, NoSuchFieldException, IllegalArgumentException,
    ...
    @SuppressWarnings("unchecked")
    @Test
    public void testProcessItEmpty()
            throws EventDeliveryException, SecurityException, NoSuchFieldException, IllegalArgumentException,
                    IllegalAccessException, InterruptedException {
        context.put(TOPICS, topic0);
        kafkaSource.configure(context);
    ...
    public void testNonExistingKafkaServer()
            throws EventDeliveryException, SecurityException, NoSuchFieldException, IllegalArgumentException,
                    IllegalAccessException, InterruptedException {
        context.put(TOPICS, topic0);
    ...
    public void testErrorCounters() throws InterruptedException, EventDeliveryException {
        ...
        doThrow(new ChannelException("dummy"))
                .doThrow(new RuntimeException("dummy"))
                .when(cp)
                .processEventBatch(any(List.class));
    ...
    public void testKafkaProperties() {
        ...
        assertEquals(
                String.valueOf(DEFAULT_AUTO_COMMIT), kafkaProps.getProperty(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG));
    ...
    public void testDefaultSettingsOnReConfigure() throws Exception {
        ...
        context.put(KafkaSourceConstants.KAFKA_CONSUMER_PREFIX + sampleConsumerProp, sampleConsumerVal);
        context.put(TOPICS, "random-topic");

        kafkaSource.configure(context);

        Assert.assertEquals(sampleConsumerVal, kafkaSource.getConsumerProps().getProperty(sampleConsumerProp));
    ...
    public void testTopicCustomHeaderSet() throws InterruptedException, EventDeliveryException {
        ...
        Status status = kafkaSource.process();
        assertEquals(Status.READY, status);
        Assert.assertEquals("hello, world2", new String(events.get(0).getBody(), Charsets.UTF_8));
    ...
    public void testTopicKafkaHeaderSet() throws InterruptedException, EventDeliveryException {
        ...
        long date = ZonedDateTime.of(2022, 10, 7, 8, 0, 0, 0, ZoneId.systemDefault())
                .toInstant()
                .toEpochMilli();
        Headers headers = new RecordHeaders();
        headers.add(new RecordHeader(correlatorHeader, "12345".getBytes(StandardCharsets.UTF_8)));
        kafkaServer.produce(topic0, 0, date, "", "hello, world2".getBytes(StandardCharsets.UTF_8), headers);
    ...
    public void testTopicCustomHeaderNotSet() throws InterruptedException, EventDeliveryException {
        ...
        Status status = kafkaSource.process();
        assertEquals(Status.READY, status);
        Assert.assertEquals("hello, world3", new String(events.get(0).getBody(), Charsets.UTF_8));
    ...
    @SuppressWarnings("unchecked")
    @Test
    public void testSslSource()
            throws EventDeliveryException, SecurityException, IllegalArgumentException, InterruptedException {
        context.put(TOPICS, topic0);
        context.put(BATCH_SIZE, "1");
    ...
    @SuppressWarnings("unchecked")
    @Test
    public void testSslWithFqdnValidationFailedSource()
            throws EventDeliveryException, SecurityException, IllegalArgumentException, InterruptedException {
        context.put(TOPICS, topic0);
        context.put(BATCH_SIZE, "1");
