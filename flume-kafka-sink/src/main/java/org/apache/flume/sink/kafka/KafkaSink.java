...
public class KafkaSink extends AbstractSink implements Configurable, BatchSizeSupported {
            ...
            for (; processedEvents < batchSize; processedEvents += 1) {
                ...
                if (allowTopicOverride) {
                    ...
                    if (eventTopic == null) {
                        ...
                        logger.debug(
                                "{} was set to true but header {} was null. Producing to {}" + " topic instead.",
                                new Object[] {KafkaSinkConstants.ALLOW_TOPIC_OVERRIDE_HEADER, topicHeader, eventTopic});
                    }
                } else {
                    eventTopic = topic;
                }
                ...
                try {
                    ...
                    if (!headerMap.isEmpty()) {
                        ...
                        for (Map.Entry<String, String> entry : headerMap.entrySet()) {
                            ...
                            if (value != null) {
                                tempHeaders.add(
                                        new RecordHeader(entry.getValue(), value.getBytes(StandardCharsets.UTF_8)));
                            }
                        }
    ...
    public void configure(Context context) {
        ...
        allowTopicOverride = context.getBoolean(
                KafkaSinkConstants.ALLOW_TOPIC_OVERRIDE_HEADER, KafkaSinkConstants.DEFAULT_ALLOW_TOPIC_OVERRIDE_HEADER);

        topicHeader = context.getString(
                KafkaSinkConstants.TOPIC_OVERRIDE_HEADER, KafkaSinkConstants.DEFAULT_TOPIC_OVERRIDE_HEADER);

        String transactionalID = context.getString(KafkaSinkConstants.TRANSACTIONAL_ID);
        if (transactionalID != null) {
            try {
                context.put(
                        KafkaSinkConstants.TRANSACTIONAL_ID,
