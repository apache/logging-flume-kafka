...
public class TestUtil {
    ...
    private void init() {
        try {
            ...
            if (externalServers) {
                ...
                kafkaServerSslUrl = settings.getProperty("kafka-server-ssl-url", kafkaServerUrl);
    ...
    private boolean startEmbeddedKafkaServer() {
        ...
        try {
            ...
            kafkaProperties.put(SSL_KEY_PASSWORD_CONFIG, "password");
            kafkaServer = new KafkaLocal(kafkaProperties);
            kafkaServer.start();
            kafkaServerUrl = kafkaServer.getBootstrapServers();
            kafkaServerSslUrl = kafkaServer.getBootstrapSslServers();
            clientProps = createClientProperties();
            logger.info("Kafka Server is successfully started on {}", kafkaServerUrl);
            return true;
