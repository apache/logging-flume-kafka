...
public class KafkaChannel extends BasicChannelSemantics {
        ...
        if (transactionalID != null) {
            try {
                ...
                throw new ConfigurationException(
                        "Unable to configure transactional id, as cannot work out hostname", e);
            }
        }
    ...
    private class KafkaTransaction extends BasicTransactionSemantics {
        ...
        protected void doPut(Event event) throws InterruptedException {
            ...
            try {
                ...
                if (partitionId != null) {
                    producerRecords
                            .get()
                            .add(new ProducerRecord<String, byte[]>(
                                    topic.get(), partitionId, key, serializeValue(event, parseAsFlumeEvent)));
                } else {
                    producerRecords
                            .get()
                            .add(new ProducerRecord<String, byte[]>(
                ...
                try {
                    ...
                    if (consumerAndRecords.get().recordIterator.hasNext()) {
                        ConsumerRecord<String, byte[]> record =
                                consumerAndRecords.get().recordIterator.next();
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "{} processed output from partition {} offset {}",
                    ...
                    logger.warn(
                            "Error while getting events from Kafka. This is usually caused by "
                                    + "trying to read a non-flume event. Ensure the setting for "
                                    + "parseAsFlumeEvent is correct",
                            ex);
            ...
            if (type.equals(TransactionType.PUT)) {
                ...
                try {
                    ...
                    if (useKafkaTransactions) {
                        ...
                        // Per the Kafka docs we do not need to linger or wait for the callback if we're using
                        // transactions
                        producer.flush();
        ...
        private byte[] serializeValue(Event event, boolean parseAsFlumeEvent) throws IOException {
            ...
            if (parseAsFlumeEvent) {
                ...
                AvroFlumeEvent e =
                        new AvroFlumeEvent(toCharSeqMap(event.getHeaders()), ByteBuffer.wrap(event.getBody()));
    ...
    private class ConsumerAndRecords {
        ...
        private String getCommittedOffsetsString() {
            ...
            Map<TopicPartition, OffsetAndMetadata> map = consumer.committed(consumer.assignment());
